...
if ($role === 'teacher' && $group['advisor_id'] == $user_id) {
    $allow = true;
} else {
...
<div class="container">
    ...
    <p><strong>อาจารย์ที่ปรึกษา:</strong>
    ...
    <?php if ($role === 'teacher'): ?>
        <p>
            <a class="btn btn-green" 
               href="meeting_create.php?group_id=<?= $group_id ?>">
               ➕ สร้างห้องพบปะใหม่
            </a>
        </p>
    <?php endif; ?>
    ...
    <table>
        ...
        <?php while ($m = $meetings->fetch_assoc()): ?>
            <tr>
                <td>สัปดาห์ที่ <?= $m['week_number'] ?></td>
                <td>
                    <?= $m['is_closed'] ? "✔ ปิดแล้ว" : "🔵 เปิดอยู่" ?>
                </td>
                <td><?= $m['started_at'] ?></td>
                <td><?= $m['ended_at'] ?: "-" ?></td>
                <td>
                    <a class="btn btn-blue"
                        href="meeting_chat.php?meeting_id=<?= $m['id'] ?>">
                        💬 เข้าห้องแชท
                    </a>
                </td>
            </tr>
    ...
    <p style="margin-top:15px;">
        <a class="btn btn-blue" href="group_chat.php?id=<?= $group_id ?>">
            ⬅ กลับไปหน้ากลุ่ม
        </a>
    </p>
</div>
